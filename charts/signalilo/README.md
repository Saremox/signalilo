...
Edit the README.md.gotmpl template instead.
-->
...
![Version: 0.1.0](https://img.shields.io/badge/Version-0.1.0-informational?style=flat-square) ![AppVersion: v0.15.0](https://img.shields.io/badge/AppVersion-v0.15.0-informational?style=flat-square)

## Migrating from appuio/charts

This chart is a drop-in replacement for the old
[appuio/charts signalilo chart](https://github.com/appuio/charts/tree/master/appuio/signalilo).
It keeps the same chart name, the same `values.yaml` keys and defaults, and
the same template helpers, so an existing release can be moved over without
changing its values file or losing its resources:

1. Add this chart's repository and point your existing release at it, using
   the same release name and namespace.
2. The only value most installs will need to change is the image location:
   the image moved from `docker.io/vshn/signalilo` to
   `ghcr.io/saremox/signalilo` (already the default in this chart's
   `values.yaml`). If you previously overrode `image.registry` or
   `image.repository`, update those to point at the new image.
3. Run `helm upgrade` as usual. Because object names (Deployment, Service,
   ServiceAccount, Secret) are derived the same way as before, Helm will
   update the existing resources in place rather than replacing them. The
   Deployment's pod template will change (new chart/app version labels),
   so expect a single rolling restart as part of the migration - this is
   the same restart you'd get from any other chart upgrade.
...
`image.repository` | Image repository | `saremox/signalilo`
...
`resources` | Pod resource requests and limits | `{requests: {cpu: 100m, memory: 64Mi}, limits: {cpu: 200m, memory: 128Mi}}`
